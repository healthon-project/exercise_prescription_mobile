document.addEventListener('DOMContentLoaded', () => {
    ...
    cards.forEach(card => {
        card.addEventListener('dragstart', (e) => {
            ...
            if (card.id === 'ex-run') draggedColorClass = 'color-run';
            else if (card.id === 'ex-swim') draggedColorClass = 'color-swim';
            else if (card.id === 'ex-bike') draggedColorClass = 'color-bike';
            else if (card.id === 'ex-jump') draggedColorClass = 'color-jump';
            else if (card.id === 'ex-stretch') draggedColorClass = 'color-stretch';
            else if (card.id === 'ex-soccer') draggedColorClass = 'color-soccer';
            else if (card.id === 'ex-basket') draggedColorClass = 'color-basket';
            else if (card.id === 'ex-dance') draggedColorClass = 'color-dance';
            else if (card.id === 'ex-walk') draggedColorClass = 'color-walk';
            else if (card.id === 'ex-strength') draggedColorClass = 'color-strength';
            else if (card.id === 'ex-pe') draggedColorClass = 'color-pe';
            else if (card.id === 'ex-other') draggedColorClass = 'color-other';
            else draggedColorClass = null;
    ...
    function appendMessage(text, isUser) {
        ...
        msgDiv.innerHTML = text; // allow basic br tags if needed
...
4. 처방전을 준 다음에는 "오늘 당장 해볼 수 있겠어?!" 같이 실천을 유도하는 질문으로 대화를 이어가.`;
    ...
    document.getElementById('btn-save').addEventListener('click', () => {
        alert('운동 계획이 저장되었습니다! (로컬 스토리지를 확인하세요)');
    });
});
